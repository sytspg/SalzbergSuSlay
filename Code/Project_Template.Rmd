...
Table 2 below communicates the findings from the `summary()` function applied to selected variables which gives us a brief understanding of the data dimensions.
...
So far we have explored the relationships between climate vulnerability and economic stability in North Carolina. We do recognize that one limitation of our study is the data sources. Since we obtained the database from an indirect source (NC Data Portal), the indices might not be under the same time frame, and thus, the findings might not be valid under all occasions. Though we both strong correlations and spatial distribution as evidence to support the findings, it would be better to look into the relationships under different spatial and temporal scales.

For further analyses, we would be interested in seeing the changes in time series data. NOAA and other weather-monitoring institutions provide reliable long-term climate and weather data collected at weather stations across the country through their online portal (https://www.ncei.noaa.gov/cdo-web/). With the time series analyses, we could better understand how climate change is affecting the local weather and whether certain counties are disproportionally affected by the changes.
