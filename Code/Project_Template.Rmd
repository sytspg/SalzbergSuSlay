...
1.  How do walkability, flood risk, and heat indices affect the average household income? 
...
This dataset describes household income and population for all 100 counties in NC as of . Each county has its own row and is associated with several climate change indicators:
...
Each indicator has its own column for a total of 14 columns. Most of the data was obtained from the NC Data Portal (https://ncdataportal.org/), a platform that gathers social and enviromental data related to North Carolina from multiple sources (e.g., CDC, Census, etc.).
...
USA counties shapefile, filtering for just the NC features (FIPS: 37). This dataset provides
...
The table below communicates the findings from the `summary()` function applied to selected variables; this gives us a brief understanding of the dimensions of the data.
...
JOSH: Among all the indices, why we chose these four? Difference/distinctions?

Referring to the plots, describe the general distributions, explain any skewness or outliers. Especially explain Fig 1B, where the average and median income data have different skewness. Why? What does that mean to the further analyses.

Also, for the right-skewed flood risk index, why do we want to apply a log transformation (to make the distribution normal)?

We explore here visually the distribution of the variables in more detail. 

From the summary above, we could identify that the distribution of population count is right skewed because the mean is larger than the median. The graph, figure 1A below, provides the proof of this. We identified the two counties with populations over 1 million as Wake and Mecklenburg Counties. The significant range in county population has implications for our analysis discussed later. 
Figure 1B shows that household incomes do not demonstrate the same absolute magnitude in range; however, average household incomes also exhibit rightward skew due to the highest income households drawing averages up. In the case of median household incomes, the range is much smaller, and is less influenced by the largest incomes in each county. Capturing this difference helps us understand the extent of income ranges between and within counties. 
Figure 1C demonstrates that approximately three quarters of counties in North Carolina have below 10% of their population living within Flood Hazard Areas. However, there are three counties with over 50% of their population residing in Flood Hazard Areas; the highest rate is, as the summary table describes above, nearly 95% of residents. To accommodate such a dramatic skew in our analysis, we will eventually apply a log transformation to normalize the distribution. 
Lastly, in Figure 1D, we explore how the count of days over the county's 95th percentile Heat Index is distributed across the state. We can see that the data approximates a normal distribution, but further analysis would be required to confirm the goodness of fit, especially given the slightly off-center mode.
