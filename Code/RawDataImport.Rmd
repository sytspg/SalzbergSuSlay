...
```{r Exploring the Structure of the Raw Data, include=TRUE, echo=FALSE}
head(RawNCData)
str(RawNCData)
summary(RawNCData)
dim(RawNCData)
...
```{r Selecting Columns and Setting as Numeric, include = TRUE}
...
NCData_Mutate <- RawNCData %>% 
  ...
  filter(County != "Report Location", County != "United States")
...
NCData_Mutate[2:12]  <- lapply(NCData_Mutate[2:12], as.numeric) 
